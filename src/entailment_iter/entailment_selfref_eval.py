...
# this function reads through a file already containing the initial solution, feedback, and subsequent iterations
# it then finds the first iterations where the solution is fixed and correct (evaluated programatically)
def evaluate_code_prompt(path, num_gsm: int = 1319):
    ...
    for i in range(5):
        print(
            f"Accuracy at attempt {i} = {df[i].sum() / num_gsm:.2%} ({df[i].sum()}/{num_gsm})"
        )
