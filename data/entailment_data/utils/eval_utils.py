...
from utils.proof_utils import (
    parse_entailment_step_proof,
    align_conclusions_across_proofs,
    rewrite_aligned_proof,
    score_sentence_overlaps,
    ruletaker_inferences_scores,
    parse_entailment_step_proof_remove_ids,
    rewrite_aligned_proof_noids,
)
...
T5_BAD_REGEX = re.compile(
    "[^" + re.escape(".".join([chr(x) for x in T5_GOOD_CHARS])) + "]"
)
...
def nlg_string_similarities(prediction, gold_strings, normalize=True):
    ...
    if INCLUDE_NLG_EVAL:
        ...
        if "CIDEr" in res:
            del res["CIDEr"]
    rouge_l_score = rouge_metric_max_over_ground_truths(
        rouge_l, prediction, gold_strings
    )
    res["ROUGE_L_F"] = rouge_l_score["rouge-l"]["f"]
    if USE_GOOGLE_ROUGE_CODE:
        res["ROUGE_L_G"] = rouge_google_metric_max_over_ground_truths(
            prediction, gold_strings
        )
    res["pred"] = prediction
...
def nlg_string_similarities_intermediates_with_F1(
    ...
    for prediction, gold in prediction_to_aligned_gold.items():
        ...
        #    del res['CIDEr']
        rouge_l_score = rouge_metric_max_over_ground_truths(
            rouge_l, prediction_norm, gold_strings
        )
        if bleurt_scorer:
            bleurt_score = bleurt_scorer.score(
                references=gold_strings, candidates=[prediction_norm], batch_size=1
            )[0]
        else:
            bleurt_score = -1
    ...
    res["ROUGE_L_F"] = sum_rouge_l_score / max(
        1, len(prediction_to_aligned_gold.keys())
    )
    res["ROUGE_L_F_perfect_align"] = sum_perfect_align_rouge_l_score / max(
        1, num_perfect_aligns
    )
    res["BLEURT"] = sum_bleurt_score / max(1, len(prediction_to_aligned_gold.keys()))
    res["BLEURT_perfect_align"] = sum_perfect_align_bleurt_score / max(
        1, num_perfect_aligns
    )
    ...
    res["BLEURT_acc_perfect_align"] = int(
        num_perfect_align_bleurt_correct == num_perfect_aligns
    )
    res["fraction_perfect_align"] = num_perfect_aligns / max(
        1, len(prediction_to_aligned_gold.keys())
    )
    res["pred"] = preds
    res["gold"] = golds
...
def nlg_string_similarities_intermediates(
    prediction_to_aligned_gold: dict,
    prediction_to_perfect_match: dict,
    normalize=True,
    bleurt_scorer=None,
    ...
    for prediction, gold in prediction_to_aligned_gold.items():
        ...
        if bleurt_scorer:
            bleurt_score = bleurt_scorer.score(
                gold_strings, [prediction], batch_size=1
            )[0]
        else:
            bleurt_score = -1
        # bleurt_score = max(0.0, min(1.0, unnorm_bleurt_score))
    ...
    res["BLEURT_acc"] = int(
        num_bleurt_correct == len(prediction_to_aligned_gold.keys())
    )
    res["BLEURT_acc_perfect_align"] = int(
        num_perfect_align_bleurt_correct == num_perfect_aligns
    )
...
def bool_accuracy(slots, gold):
    ...
    res = {
        "acc": float(pred_answer == gold_answer),
...
SCORING_SPECS = {"ruletaker_inferences": ruletaker_inferences_scores}
...
def collate_scores(predictions):
    ...
    averaged_metrics = [
        ...
        "BLEURT_acc",
        "BLEURT_acc_perfect_align",
        "acc_perfect_align",
        "fraction_perfect_align",
        "edited_or_not_acc",
        "num_fever_queries",
        "num_fever_queries_no_answer",
        "num_fever_queries_no_rationale",
        "num_premises",
        "num_premises_valid",
        "num_premises_valid_support",
        "percent_valid_premises",
    ]
    ...
    for pred in predictions:
        ...
        metrics_by_angle[angle]["counter"] = (
            metrics_by_angle[angle].get("counter", 0) + 1
        )
            ...
            for metric in averaged_metrics + aggregated_metrics:
                if metric in slot_metrics:
                    metrics_by_angle[angle][slot][metric] = (
                        metrics_by_angle[angle][slot].get(metric, 0)
                        + slot_metrics[metric]
                    )
...
def score_aligned_entail_tree_proof(
    prediction, gold_list, angle, gold_json_record: dict, bleurt_scorer=None
):
    res = {}
    ...
    (
        sentences_pred,
        inferences_pred,
        int_to_all_ancestors_pred,
        relevant_sentences_pred,
        id_to_int_pred,
    ...
    (
        sentences_gold,
        inferences_gold,
        int_to_all_ancestors_gold,
        relevant_sentences_gold,
        id_to_int_gold,
    ...
    sentences_pred_aligned = rewrite_aligned_proof(
        prediction, pred_int_to_gold_int_mapping
    )
    print(f"\n\n\n++++++++++++++++++++++++++++++++++++")
    ...
    res[angle + "-leaves"] = score_sentence_overlaps(
        sentences=sorted(list(relevant_sentences_pred)),
        sentences_gold=sorted(list(relevant_sentences_gold)),
    )

    res[angle + "-steps"] = score_sentence_overlaps(
        sentences=sorted(list(sentences_pred_aligned)),
    ...
    res[angle + "-intermediates"] = nlg_string_similarities_intermediates_with_F1(
        prediction_to_aligned_gold=prediction_to_aligned_gold,
    ...
    res[angle + "-overall"] = overall_proof_score(
        leaves=res[angle + "-leaves"],
...
def score_aligned_entail_tree_proof_onlyIR(
    prediction,
    gold_list,
    angle,
    gold_json_record: dict,
    pred_json_record: dict,
    bleurt_scorer=None,
):
    res = {}
    if gold_list is None:
    ...
    (
        ...
        prediction, slot_json_record=pred_json_record
    )
    ...
    (
        ...
        gold_list[0], slot_json_record=gold_json_record
    )
    ...
    (
        ...
        sentences_pred_aligned_strings,
    ) = rewrite_aligned_proof_noids(
        prediction,
        pred_int_to_gold_int_mapping,
    ...
    print(f"*********sentences_gold:{sentences_gold}")
    res[angle + "-steps"] = score_sentence_overlaps(
        sentences=sorted(list(sentences_pred_aligned_strings)),
        sentences_gold=sorted(list(sentences_gold)),
    )
    res[angle + "-steps"]["pred_to_gold_mapping"] = pred_int_to_gold_int_mapping
    res[angle + "-steps"]["sentences_pred_aligned"] = sentences_pred_aligned
...
def score_prediction_whole_proof(
    prediction,
    gold,
    ...
    """prediction is dict with following keys:
    "id": angle_data[line_idx]["id"] - id of corresponding entailment problem,
    "angle": angle_data[line_idx]["angle"] - angle of corresponding entailment problem,
    "prediction": line.strip() - actual entailment tree prediction string,
    'angle_str': str - shortform_angle(angle, sort_angle=False),
    'slots': decompose_slots(pred["prediction"]),
    """
    """gold is dict with following keys:
        look at data/public_dataset/.../dataset/task_1/train.json for an example of gold object
    """
    """
        prediction_json = angle_base_name (e.g. os.path.basename(data/processed_data/angles/task_1/ = ""))
    """
    """
        scoring_spec={
                    "hypothesis_eval": "nlg",
    ...
    for angle in angles_out:
        ...
        if angle == "hypothesis":
            if hypothesis_eval == "old_emf1":
                ...
                res[angle] = score_sentence_overlaps(
                    sentences=sentences_pred, sentences_gold=sentences_gold
                )
            ...
            if proof_eval == "entail_whole_proof_align_eval":
                ...
                res.update(
                    score_aligned_entail_tree_proof(
                        slot,
                        gold_list,
                        angle,
                ...
                # This writes in multiple metrics
                res.update(
                    score_aligned_entail_tree_proof_onlyIR(
                        slot,
                        gold_list,
                        angle,
                        gold_json_record=gold,
