...
def get_args():
    ...
    parser.add_argument(
        ...
        required=True,
        type=str,
        help="Which split (train/dev/test) to evaluate.",
    )
    parser.add_argument(
        "--prediction_file",
        default=None,
...
def score_predictions(
    ...
    # each item in gold_data is just the dictionary file from the jsonl
    gold_data = load_jsonl(gold_file)

    # dict of each gold item labeled by their id
    ...
    with open(f"{score_file}.json", "w") as score_file, open(
        predictions_file, "r"
    ) as preds_file:
        for line_idx, line in tqdm(enumerate(preds_file)):
            ...
            metrics = score_prediction_whole_proof(
                ...
                prediction_json=dataset,  # IS THIS CORRECT? THIS IS WHATS BEING CALLED BASED ON POSITIONAL ARGS
                scoring_spec={
                    "hypothesis_eval": "nlg",
            ...
            gold_ints = goldslot_record.get("meta", dict()).get(
                ...
                for step in gold_proof_steps:
                    ...
                    if step.strip() and len(step.split(" -> ")) == 2:
                        ...
                        for lid in lhs_ids:
                            ...
                            print(f"step:{step}")
                            gold_proof_str_list.append(
                                f"{lhs_id}: {gold_triples[lhs_id]} &"
                            )
            ...
            if True:
                ...
                for step in pred_proof_steps:
                    ...
                    if step.strip() and len(step.split(" -> ")) == 2:
                        ...
                        for lid in lhs_ids:
                            lhs_id = lid.strip()
                            pred_proof_str_list.append(
                                f"{lhs_id}: {pred_triples.get(lhs_id, 'NULL')} &"
                            )
    ...
    print(
        f"num_dev_answers_seen_in_train_context:{num_dev_answers_seen_in_train_context}"
    )
    print(
        f"num_dev_answers_seen_in_train_answers:{num_dev_answers_seen_in_train_answers}"
    )
...
def main(args):
    ...
    if "," in prediction_files:
        ...
        prediction_files = [
            f"{dir_path}/{f}"
            for f in os.listdir(prediction_files)
            if re.match(r".*_predictions", f)
        ]
    else:
    ...
    angle_base_name = os.path.basename(
        angle_data_dir
    )  # basename of directory is just ''
    slot_file = os.path.join(slot_data_dir, args.split + ".jsonl")
    if not os.path.exists(slot_file):
        if args.split == "val" and os.path.exists(
            os.path.join(slot_data_dir, "dev.jsonl")
        ):
    ...
    if not predictions_jsonl_format:
        ...
        if not os.path.exists(angle_file):
            if args.split == "val" and os.path.exists(
                os.path.join(angle_data_dir, "dev.jsonl")
            ):
    ...
    for prediction_file in prediction_files:
        ...
        metrics_str = "\t".join(
            [
                ...
                str(
                    round(aggr_metrics["proof-intermediates"]["BLEURT_acc"] * 100.0, 2)
                ),
